// Admin Panel JavaScript - WORKING VERSION
// Put this in: public/admin-panel.js

console.log("✅ Admin Panel v5.0 Loading");
...
// EXTRACT CUSTOMER - Works with ANY format
const extractCustomer = (o) => {
  ...
  const notes = c.notes || o.customerNotes || o.note || "";
...
const renderOrders = () => {
  ...
  ["all","new","confirmed","delivered","cancelled"].forEach(s => {
    const b = $(`badge-${s}`);
    if (b) b.textContent = s === "all" ? allOrders.length : allOrders.filter(o => o.status === s).length;
  });
  ...
  grid.innerHTML = filtered.map(o => {
    ...
    return `<div class="order-card">
      <div class="order-header">
        ...
        ${badges[o.status] || badges.new}
      </div>
      <div class="order-body">
        <div class="customer-info">
          <div class="field"><svg width="13" height="13" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg><strong>${esc(c.name)}</strong></div>
...
const renderAnalytics = () => {
  ...
  orders.forEach(o => {
    const c = extractCustomer(o);
    if (c.name !== "—") uniqueCustomers.add(c.name);
  });
